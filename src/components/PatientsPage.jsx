...
import { CalendarPlus, CircleDollarSign, ClipboardPlus, Edit3, FileHeart, MessageCircle, Phone, Plus, Search, Trash2, UserRound, X } from "lucide-react";
...
import { createPatientNote, deletePatientMedia, uploadPatientMedia } from "../lib/patient-media";
...
function PatientDashboard({ patientId, summary, onBack, onBook, onChanged }) {
  ...
  return <section className="patient-dashboard">
    ...
    <header className="patient-hero"><div className="patient-avatar"><UserRound size={30} /></div><div className="patient-identity"><small>ملف #{patient.file_no}</small><h2>{patient.full_name}</h2><p><Phone size={14} /> <span dir="ltr">{patient.phone}</span>{patient.national_id && ` · ${patient.national_id}`}</p></div><div className="patient-actions"><button className="primary-button" onClick={() => onBook(patient.id)}><CalendarPlus size={17} />حجز</button><button className="secondary-button" onClick={() => setDialog("note")}><ClipboardPlus size={17} />متابعة</button><button className="secondary-button" onClick={() => setDialog("medicalNote")}><FileHeart size={17} />ملاحظة طبية</button><button className="secondary-button" onClick={() => setDialog("payment")}><CircleDollarSign size={17} />دفعة</button>{whatsappPhone && <a className="secondary-button" href={`https://wa.me/${whatsappPhone}`} target="_blank" rel="noreferrer"><MessageCircle size={17} />واتساب</a>}<button className="secondary-button" onClick={() => setDialog("edit")}><Edit3 size={17} />تعديل</button></div></header>
    ...
    <PatientTab tab={tab} data={data} onUpload={() => setDialog("media")} onRefresh={refresh} />
...
function PatientTab({ tab, data, onUpload, onRefresh }) {
  const { patient, appointments, sessions, packages, transactions, activities, patientNotes, media } = data;
  ...
  if (tab === "medical") return <div className="detail-grid"><InfoCard title="الحالات الطبية" rows={[["الحالات", patient.medical_conditions], ["الحساسية", patient.allergies], ["الأدوية", patient.medications], ["موانع العلاج", patient.contraindications], ["ملاحظات", patient.notes]]} /><article className="info-card"><h3>سجل الملاحظات الطبية</h3><DataList empty="لا توجد ملاحظات طبية متسلسلة" rows={patientNotes.map(item=>({title:item.body,meta:`${formatDate(item.occurred_at,true)} · ${item.note_type}`}))}/></article></div>;
  if (tab === "photos") return <section className="patient-media"><header><div><h3><FileHeart size={20}/>صور قبل وبعد والمرفقات</h3><p>الملفات خاصة وتُفتح بروابط مؤقتة.</p></div><button className="primary-button" onClick={onUpload}><Plus size={17}/>رفع ملف</button></header><div className="media-grid">{media.map(item=><article key={item.id}>{item.mime_type.startsWith("image/")&&item.url?<img src={item.url} alt={item.caption||item.file_name}/>:<a href={item.url||"#"} target="_blank" rel="noreferrer"><FileHeart size={35}/><span>{item.file_name}</span></a>}<div><strong>{item.media_type==="before"?"قبل":item.media_type==="after"?"بعد":"مرفق"}</strong><small>{item.caption||item.file_name} · {formatDate(item.captured_on)}</small><button aria-label="حذف الملف" onClick={async()=>{if(confirm("حذف الملف؟")){await deletePatientMedia(item);await onRefresh()}}}><Trash2 size={16}/></button></div></article>)}</div>{!media.length&&<p className="empty-line">لا توجد صور أو مرفقات بعد.</p>}</section>;
  const timeline = [
    ...
    ...patientNotes.map((item) => ({ date: item.occurred_at, title: item.body, meta: `ملاحظة طبية · ${item.note_type}` })),
...
function PatientDialog({ type, patient, appointments, onClose, onSaved }) {
  ...
  const [medicalNote, setMedicalNote] = useState({ noteType: "medical", body: "", occurredAt: new Date().toISOString().slice(0,16), sessionId: "" });
  const [media, setMedia] = useState({ file: null, mediaType: "before", caption: "", capturedOn: new Date().toISOString().slice(0,10), sessionId: "" });
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (type === "medicalNote") await createPatientNote(patient.id, medicalNote);
      if (type === "media") { if (!media.file) throw new Error("اختاري ملفًا"); await uploadPatientMedia(patient.id, media.file, media); }
      onSaved();
    } catch (saveError) { setError(saveError?.message || "تعذر الحفظ"); }
    finally { setBusy(false); }
  }

  const title = type === "note" ? "إضافة متابعة" : type === "payment" ? "تسجيل دفعة" : type === "medicalNote" ? "ملاحظة طبية" : type === "media" ? "رفع صورة أو مرفق" : "تعديل بيانات المريضة";
  return <div className="modal-backdrop"><section className="patient-dialog" role="dialog" aria-modal="true"><header><h2>{title}</h2><button className="icon-button" onClick={onClose}><X size={20} /></button></header><form onSubmit={submit}>
    ...
    {type === "medicalNote" && <div className="form-grid"><div className="field"><label>النوع</label><select value={medicalNote.noteType} onChange={e=>setMedicalNote(c=>({...c,noteType:e.target.value}))}><option value="medical">طبية</option><option value="aftercare">تعليمات ما بعد الجلسة</option><option value="general">عامة</option></select></div><div className="field"><label>الوقت</label><input type="datetime-local" value={medicalNote.occurredAt} onChange={e=>setMedicalNote(c=>({...c,occurredAt:e.target.value}))}/></div><div className="field field-wide"><label>الملاحظة</label><textarea rows="4" value={medicalNote.body} onChange={e=>setMedicalNote(c=>({...c,body:e.target.value}))} required/></div></div>}
    {type === "media" && <div className="form-grid"><div className="field"><label>نوع الملف</label><select value={media.mediaType} onChange={e=>setMedia(c=>({...c,mediaType:e.target.value}))}><option value="before">صورة قبل</option><option value="after">صورة بعد</option><option value="document">مرفق/PDF</option></select></div><div className="field"><label>التاريخ</label><input type="date" value={media.capturedOn} onChange={e=>setMedia(c=>({...c,capturedOn:e.target.value}))}/></div><div className="field field-wide"><label>الملف (حتى 10MB)</label><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={e=>setMedia(c=>({...c,file:e.target.files?.[0]||null}))} required/></div><div className="field field-wide"><label>وصف</label><input value={media.caption} onChange={e=>setMedia(c=>({...c,caption:e.target.value}))}/></div></div>}
